...
// Reordena os cards da seção "Na Mídia" (BAC_130). Recebe a lista de ids na
// ordem desejada e grava `ordem` = índice. Protegido pelo login do /adm.
...
export async function POST(req: Request) {
  ...
  await prisma.$transaction(
    ids.map((id, i) =>
      prisma.midiaItem.update({ where: { id }, data: { ordem: i + 1 } }),
    ),
  );
